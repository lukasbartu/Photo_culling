...
def update_scores(sim_data, image_scores, s_t, img, img_num, selection, s_c_ratio):
    if selection:
        sim_penalty = 0.8
    else:
        sim_penalty = 0
    ...
    for temp in sim_data:
        if img == temp["first_img"]:
            in_window = True
            if (s_c_ratio * temp["feature_similarity_score"] + (1-s_c_ratio) * temp["content_similarity_score"]) >= s_t:
                i = temp["second_id"]
...
def select_summary(sim_pth, q_pth, num, s_t, t_a_ratio, s_c_ratio, q_cutoff, size=10, selection=False):
    ...
    while selected < select_num:
        if selected != 0:
            image_scores, num = update_scores(sim_data, image_scores, s_t, added_img, num, selection, s_c_ratio)
